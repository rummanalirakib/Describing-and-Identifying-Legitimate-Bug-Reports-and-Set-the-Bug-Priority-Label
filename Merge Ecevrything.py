...
df_validRate = pd.DataFrame(validRate.values())
df_bugNum = pd.DataFrame(bugNum.values())
df_recentBugNum = pd.DataFrame(recentBugNum.values())

# Concatenate the DataFrames horizontally
merged_df1 = pd.concat([df_validRate, df_bugNum, df_recentBugNum], axis=1)
# Iterate through the DataFrame
for i in range(0, len(merged_df1)):
    for j in range(0, 3):  # Assuming you want to check the first three columns
        if pd.isna(merged_df1.iloc[i, j]):
            merged_df1.iat[i, j] = 0
...
merged_df = pd.concat([df_multinomial_nb, df_complement_nb, df_discriminative_multinomial_nb, df_nb_classifier, df_multinomial_nb_desc, df_complement_nb_desc, df_discriminative_multinomial_nb_desc, merged_df1], axis=1)
print(merged_df)
